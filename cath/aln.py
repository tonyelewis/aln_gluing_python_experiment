...
class alignment:
	def __init__(self, num_entries: int):
		...
		self.equiv_rows                   : List[List[int]] = []
		self.aln_index_by_index_per_entry : List[List[int]] = [[] for _ in range(num_entries)]
	...
	def append(self, equivs, score: float):
...
def _ssap_aln_line_empty_parts():
	return [ '0', '0', '0', '0' ]

def _left_is_present(ssap_aln_line_parts):
	return ssap_aln_line_parts[ :4 ] != _ssap_aln_line_empty_parts()

def _right_is_present(ssap_aln_line_parts):
	return ssap_aln_line_parts[ 5: ] != _ssap_aln_line_empty_parts()

def _get_left_residue(ssap_aln_line_parts):
	return pdb_residue(
	 ...
	 ) if _left_is_present( ssap_aln_line_parts ) else None

def _get_right_residue(ssap_aln_line_parts):
	return pdb_residue(
	 ...
	 ) if _right_is_present( ssap_aln_line_parts ) else None




def _process_ssap_aln_line_parts(ssap_aln_line_parts: List[str]) -> ssap_aln_entry:
	both_sides_present = _left_is_present( ssap_aln_line_parts ) and _right_is_present( ssap_aln_line_parts )
	return ssap_aln_entry(
		left_residue  = _get_left_residue ( ssap_aln_line_parts ),
		right_residue = _get_right_residue( ssap_aln_line_parts ),
		score         = float( ssap_aln_line_parts[ 4 ] ) if both_sides_present else None,
	)

def _process_ssap_aln_line(ssap_aln_line: str) -> ssap_aln_entry:
	...
	return _process_ssap_aln_line_parts( ssap_aln_line_parts )

def _get_index_based_equiv_rows(parsed_ssap_aln_data):
	left_ctr  = 0
	right_ctr = 0
	result    = []
...
def read_ssap_alignment(aln_filename) -> Tuple[ pdb_sequence, pdb_sequence, alignment ]:
	with open( aln_filename, 'r' ) as python_source_file:
		parsed_ssap_aln_data = [ _process_ssap_aln_line( i ) for i in python_source_file.readlines() if not i.startswith( '#' ) ]
	...
	result_alignment = alignment( 2 )
	for entry in _get_index_based_equiv_rows( parsed_ssap_aln_data ):
		score = entry.pop()
		# score = 1 # ************************ DON'T LEAVE THIS LIKE THIS !!! ***********************************
		result_alignment.append( entry, score )
