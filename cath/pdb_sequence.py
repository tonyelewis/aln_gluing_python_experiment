...
def _residue_id_of_residue_dict(residue: pdb_residue) -> str:
	return (
...
class pdb_sequence:
	def __init__(self, prm_residues : Iterable[pdb_residue] = [] ) -> None:
		self.residues = list( prm_residues ) # type: List
	...
	def res_ids(self):
		return [ _residue_id_of_residue_dict( x ) for x in self.residues ]
